...
def regulated_cascode_current_mirror(
    ...
    source_XMB4_via = RegulatedCM << viam2m3
    source_XMB4_via.move(RegulatedCM.ports["rcm_XMB4_multiplier_0_source_E"].center)#.movey(0.8*maxmet_sep)
    gate_XMB4_via = RegulatedCM << viam2m3
    gate_XMB4_via.move(RegulatedCM.ports["rcm_XMB4_multiplier_0_gate_S"].center)#.movey(-0.8*maxmet_sep) 
    
    drain_XMB5_via = RegulatedCM << viam2m3
    #drain_XMB5_via.move(RegulatedCM.ports["rcm_XMB5_multiplier_0_drain_W"].center)#.movey(-0.8*maxmet_sep) #.movex(1*maxmet_sep)
    drain_XMB5_via.move(fet_XMB5_ref.ports["drain_W"].center)
    source_XMB5_via = RegulatedCM << viam2m3
    #source_XMB5_via.move(RegulatedCM.ports["rcm_XMB5_multiplier_0_source_E"].center)#.movey(0.8*maxmet_sep)
    source_XMB5_via.move(fet_XMB5_ref.ports["source_E"].center)
    gate_XMB5_via = RegulatedCM << viam2m3
    #gate_XMB5_via.move(RegulatedCM.ports["rcm_XMB5_multiplier_0_gate_S"].center)#.movey(-0.8*maxmet_sep)
    gate_XMB5_via.move(fet_XMB5_ref.ports["gate_S"].center)

    drain_XM2_via = RegulatedCM << viam2m3
    drain_XM2_via.move(fet_XM2_ref.ports["drain_W"].center)#.movey(-0.8*maxmet_sep) #.movex(1*maxmet_sep)        
    gate_XM2_via = RegulatedCM << viam2m3
    gate_XM2_via.move(fet_XM2_ref.ports["gate_S"].center)#.movex(0.8*maxmet_sep) 
    ...
    RegulatedCM << c_route(pdk, fet_XM2_ref.ports["gate_W"], fet_XM2_ref.ports["drain_W"])
    #RegulatedCM << CONNECT THE SOURCE OF XM2 TO VDD / TOP PORT VSS
    #RegulatedCM << CONNECT THE SOURCE OF XMB4 TO VSS/ TOP PORT VSS
    ...
    psize = (0.5,0.5)
    move_info = list()
    iref_label = rectangle(layer = pdk.get_glayer("met3_pin"), size=psize, centered=True).copy()
    iref_label.add_label(text="IREF", layer=pdk.get_glayer("met3_label"))
    ...
    # move_info.append((vdd_label, RegulatedCM.ports["rcm_XM2_source_N"], None))
    move_info.append((vdd_label, fet_XM2_ref.ports["source_N"], None))  
    ...
    # move_info.append((vss_label, RegulatedCM.ports["rcm_XMB5_source_W"], None))
    move_info.append((vss_label, fet_XMB5_ref.ports["source_W"], None)) 
    ...
    #top_level.show()
    RegulatedCM.show()
    #display_component(RegulatedCM, scale = 5,path="../../")
