...
def current_mirror_base(
    ...
    dist_DS = abs(top_level.ports[f"currm_A_0_drain_E"].center[0] - top_level.ports[f"currm_A_0_drain_W"].center[0])/2

    dist_GS = abs(top_level.ports[f"currm_A_0_gate_E"].center[0] - top_level.ports[f"currm_A_0_gate_W"].center[0])/2

    bottomA_drain_via  = top_level << viam2m3

    bottomA_drain_via.move(top_level.ports[f"currm_A_0_drain_W"].center).movex(dist_DS)

    bottomA_gate_via  = top_level << viam2m3

    bottomA_gate_via.move(top_level.ports[f"currm_A_0_gate_W"].center).movex(dist_GS)

    bottomB_gate_via  = top_level << viam2m3

    bottomB_gate_via.move((bottomA_gate_via.center[0], top_level.ports[f"currm_B_0_gate_W"].center[1]))
    ...
    try:
        pass
        top_level << straight_route(pdk, top_level.ports["currm_A_0_dummy_L_gsdcon_top_met_W"],top_level.ports["welltie_W_top_met_W"],glayer2="met1")
        
        top_level << straight_route(pdk, top_level.ports[f'currm_B_0_dummy_R_gsdcon_top_met_E'], top_level.ports["welltie_E_top_met_E"], glayer2="met1")
