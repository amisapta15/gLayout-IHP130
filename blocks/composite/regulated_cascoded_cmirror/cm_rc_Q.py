...
def generate_regulated_current_mirror_netlist(
    ...
    **kwargs,
    ...
    nodes = list(
        set([drain_net_in, drain_net_aux, gate_net, drain_net_out, source_net_in, source_net_out, bulk_net])
    )
    ...
    source_netlist += (
        f"XA {drain_net_aux} {B_source_C_drain_net} {bulk_net} {bulk_net} "
        f"{model_name} l={length} w={width} m={mtop}\n"
    )
    source_netlist += (
        f"XB {drain_net_in} {drain_net_aux} {B_source_C_drain_net} {bulk_net} "
        f"{model_name} l={length} w={width} m={mtop}\n"
    )
    source_netlist += (
        f"XC {B_source_C_drain_net} {drain_net_in} {bulk_net} {bulk_net} "
        f"{model_name} l={length} w={width} m={mtop}\n"
    )
    source_netlist += (
        f"XD {E_source_D_drain_net} {drain_net_in} {bulk_net} {bulk_net} "
        f"{model_name} l={length} w={width} m={mtop}\n"
    )
    source_netlist += (
        f"XE {drain_net_out} {drain_net_aux} {E_source_D_drain_net} {bulk_net} "
        f"{model_name} l={length} w={width} m={mtop}\n"
    )
    source_netlist += (
        f"XF {drain_net_aux} {E_source_D_drain_net} {bulk_net} {bulk_net} "
        f"{model_name} l={length} w={width} m={mtop}\n"
    )
